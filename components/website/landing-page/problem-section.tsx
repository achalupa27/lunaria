...
const ProblemSection = () => {
    return (
        <section className='flex flex-col items-center justify-center pb-24 text-center'>
            ...
            <motion.p initial={{ y: 30, opacity: 0 }} viewport={{ once: true }} whileInView={{ y: 0, opacity: 1 }} transition={{ duration: 1.5, delay: 0.3 }} className='max-w-2xl text-lg text-zinc-500'>
                Most finance apps track spending but overlook wealth growth essentials like savings and income.
            </motion.p>
            <div className='mt-12 flex flex-col items-stretch space-y-8 lg:flex-row lg:space-x-12 lg:space-y-0'>
                <motion.div initial={{ y: 30, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ duration: 1.2, delay: 0.6 }} viewport={{ once: true }} className='flex-1'>
                    <Card className='flex h-full flex-col items-center justify-center px-8 py-10'>
                        <div className='mb-3 text-2xl font-medium'>Lack of Insights</div>
                        <p className='text-zinc-500'>Traditional apps show numbers but don’t provide deep insights.</p>
                    </Card>
                </motion.div>
