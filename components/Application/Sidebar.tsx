    ...
    return (
        <div className='flex h-screen w-14 flex-col items-center justify-center space-y-4  border-r bg-white py-2 shadow-sm dark:border-0 dark:bg-primary '>
            <div className='flex flex-1 flex-grow flex-col space-y-3'>
                <nav className={`sidebar-button p-2 ${tab === 'Dashboard' ? 'bg-white/20' : null}`} onClick={() => dispatch(setTab('Dashboard'))}>
                    <Image src={getDashboardLogo() as string} alt='' height={48} width={48} />
                </nav>
                <div className={`sidebar-button ${tab === 'Make' ? 'bg-white/20' : null}`} onClick={() => dispatch(setTab('Make'))}>
                    <Image src={getMakeLogo() as string} alt='' height={48} width={48} />
                </div>
                <div className={`sidebar-button ${tab === 'Save' ? 'bg-white/20' : null}`} onClick={() => dispatch(setTab('Save'))}>
                    <Image src={getSaveLogo() as string} alt='' height={48} width={48} />
                </div>
                <div className={`sidebar-button ${tab === 'Spend' ? 'bg-white/20' : null}`} onClick={() => dispatch(setTab('Spend'))}>
                    <Image src={getSpendLogo() as string} alt='' height={48} width={48} />
                </div>
            </div>

            <div className='flex-col items-center space-y-3'>
                <div className={`sidebar-button p-2 ${tab === 'Settings' ? 'text-primary dark:text-primary-dark' : null}`} onClick={() => dispatch(setTab('Settings'))}>
